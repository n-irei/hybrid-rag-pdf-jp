...
日本語PDFに対応したRAG（検索拡張生成）システムです。
PDFのテキスト抽出にハイブリッド方式を採用しており、PyMuPDF（fitz）による直接抽出を優先し、テキストが取得できない画像ベースのPDFは Claude Vision API にフォールバックします。抽出したテキストは ChromaDB + nomic-embed-text でベクトル検索し、Claude APIで日本語回答を生成します。
...
PDF ファイル
    ...
    ▼
【ハイブリッドテキスト抽出】
    ├─ PyMuPDF（fitz）で直接テキスト抽出を試みる
    │       │
    │       ├─ 成功（テキストあり）→ そのまま使用
    │       │
    │       └─ 失敗（画像ベースPDF）→ Claude Vision API（claude-opus-4-6）でOCR
    │
    ▼
